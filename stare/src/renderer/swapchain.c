...
void pelagia_assemble_swapchain(struct pelagia_assemble_swapchain_work *work)
{
    ...
    b32                 dissasemble     = work->dissasemble;
    ...
    if (dissasemble || old_sc != VK_NULL_HANDLE)
        vulkan_clear_swapchain(swapchain, device);
    if (dissasemble) {
        if (swapchain->sc)
            device->api.vkDestroySwapchainKHR(device->logical, swapchain->sc, NULL);
        if (swapchain->surface)
            vk->api.vkDestroySurfaceKHR(vk->instance, swapchain->surface, NULL);
