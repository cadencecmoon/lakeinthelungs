    ...
    u32                     application_version)
{
    ...
    if (extension_count > 0) {
        VkExtensionProperties *extension_props = (VkExtensionProperties *)iascratch(NULL, sizeof(VkExtensionProperties) * extension_count, 1);
        ...
        instance_extensions = (char **)iascratch(NULL, sizeof(char *) * extension_count, 1);
...
static void select_rendering_device(struct pelagia_renderer_init_work *work)
{
    ...
    VkPhysicalDevice *physical_devices = (VkPhysicalDevice *)iascratch(NULL, sizeof(VkPhysicalDevice) * physical_device_count, 1);
    ...
    for (u32 i = 0; i < physical_device_count && pelagia->device.physical == VK_NULL_HANDLE; i++) {
        ...
        queue_family_properties = (VkQueueFamilyProperties *)iascratch(NULL, queue_family_count * sizeof(VkQueueFamilyProperties) * queue_family_count, 1);
        ...
        if (preferred_device_idx == (s32)i || (physical_device_properties.deviceType == VK_PHYSICAL_DEVICE_TYPE_DISCRETE_GPU && preferred_device_idx < 0) || physical_device_count == 1) {
            extension_properties = (VkExtensionProperties *)iascratch(NULL, sizeof(VkExtensionProperties) * extension_count, 1);
            ...
            for (u32 j = 0; j < extension_count; j++) {
                ...
                if (physical_device_properties.apiVersion < VK_MAKE_API_VERSION(0,1,4,0)) {
                    ...
                    } else if (!strcmp(extension_properties[j].extensionName, "VK_KHR_maintenance5")) {
                        extension_bits |= vulkan_extension_maintenance5_bit;
                    }
                }
                if (physical_device_properties.apiVersion < VK_MAKE_API_VERSION(0,1,3,0)) {
                    ...
                    } else if (!strcmp(extension_properties[j].extensionName, "VK_KHR_maintenance4")) {
                        extension_bits |= vulkan_extension_maintenance4_bit;
                    }
                }
                if (physical_device_properties.apiVersion < VK_MAKE_API_VERSION(0,1,2,0)) {
                    if (!strcmp(extension_properties[j].extensionName, "VK_KHR_buffer_device_address")) {
                        extension_bits |= vulkan_extension_buffer_device_address_bit;
                    } else if (!strcmp(extension_properties[j].extensionName, "VK_KHR_draw_indirect_count")) {
                        extension_bits |= vulkan_extension_draw_indirect_count_bit;
                    } else if (!strcmp(extension_properties[j].extensionName, "VK_EXT_descriptor_indexing")) {
                        extension_bits |= vulkan_extension_descriptor_indexing_bit;
                    } else if (!strcmp(extension_properties[j].extensionName, "VK_EXT_host_query_reset")) {
                        extension_bits |= vulkan_extension_host_query_reset_bit;
                    } else if (!strcmp(extension_properties[j].extensionName, "VK_KHR_timeline_semaphore")) {
                        extension_bits |= vulkan_extension_timeline_semaphore_bit;
                    } else if (!strcmp(extension_properties[j].extensionName, "VK_KHR_vulkan_memory_model")) {
                        extension_bits |= vulkan_extension_vulkan_memory_model_bit;
                    }
                }
            }
    ...
    u32             thread_count)
{
    ...
    extensions = (char **)iascratch(NULL, sizeof(char *) * extension_count, 1);
    ...
    if (device->extensions & vulkan_extension_maintenance5_bit)
        extensions[i++] = "VK_KHR_maintenance5";
    if (device->extensions & vulkan_extension_dynamic_rendering_bit)
        extensions[i++] = "VK_KHR_dynamic_rendering";
    if (device->extensions & vulkan_extension_maintenance4_bit)
        extensions[i++] = "VK_KHR_maintenance4";
    if (device->extensions & vulkan_extension_buffer_device_address_bit)
        extensions[i++] = "VK_KHR_buffer_device_address";
    if (device->extensions & vulkan_extension_draw_indirect_count_bit)
        extensions[i++] = "VK_KHR_draw_indirect_count";
    if (device->extensions & vulkan_extension_descriptor_indexing_bit)
        extensions[i++] = "VK_EXT_descriptor_indexing";
    if (device->extensions & vulkan_extension_host_query_reset_bit)
        extensions[i++] = "VK_EXT_host_query_reset";
    if (device->extensions & vulkan_extension_timeline_semaphore_bit)
        extensions[i++] = "VK_KHR_timeline_semaphore";
    if (device->extensions & vulkan_extension_vulkan_memory_model_bit)
        extensions[i++] = "VK_KHR_vulkan_memory_model";
    ...
    u32 api_version = device->physical_properties.apiVersion;
    ...
    result = vulkan_load_device_api_procedures(&vk->api, &device->api, device->logical, api_version, device->extensions);
    if (result != result_success) {
        log_error("Can't load the Vulkan API procedures for device idx.");
        return result;
    }

    /* enable promoted extension bits, to enable checking for availability */
    if (api_version >= VK_MAKE_API_VERSION(0,1,2,0))
        device->extensions |= vulkan_extension_mask_api_12;
    if (api_version >= VK_MAKE_API_VERSION(0,1,3,0))
        device->extensions |= vulkan_extension_mask_api_13;
    if (api_version >= VK_MAKE_API_VERSION(0,1,4,0))
        device->extensions |= vulkan_extension_mask_api_14;
    ...
    log_info("       Api version : %u.%u.%u", (api_version >> 22U), (api_version >> 12U) & 0x3ffU, (api_version & 0xfffU));
...
void pelagia_renderer_init(struct pelagia_renderer_init_work *work)
{
    if (!work) return;
    ...
    struct pelagia_renderer_fini_work fini = { .pelagia = pelagia };
    ...
    result = create_instance(&pelagia->vk, work->application_name, work->application_version);
    if (result != result_success) {
    ...
    u32 flags = pelagia_flag_initialized;
    if (work->enable_vsync)
        flags |= pelagia_flag_vsync_enabled;

    at_fetch_or_relaxed(&pelagia->flags, flags);
    arena_init(&pelagia->swapchain.arena, 490);
}

void pelagia_renderer_fini(struct pelagia_renderer_fini_work *work)
{
    ...
    if (device->logical != VK_NULL_HANDLE) {
        ...
        if (device->compute_queues && device->compute_queue_count > 0 && device->compute_queue_family_idx != device->graphics_queue_family_idx)
            free(device->compute_queues); /* TODO ia */
        if (device->transfer_queues && device->transfer_queue_count > 0 && device->transfer_queue_family_idx != device->graphics_queue_family_idx)
            free(device->transfer_queues); /* TODO ia */
