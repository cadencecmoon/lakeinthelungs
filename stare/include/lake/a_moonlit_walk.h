...
/** Controls what work should be done in this context. */
enum amw_workload_context {
    amw_workload_context_startup = 0,
    amw_workload_context_continue,
    amw_workload_context_exit
};

/** Holds per-frame memory, pointer to the engine context and data needed to calculate and present a frame. */
struct amw_workload {
    enum amw_workload_context   context;
    u32                         idx;
    f64                         dt;
...
typedef s32  (*PFN_amw_init)(struct a_moonlit_walk *AMW, struct amw_workload *work_array, u32 frame_count, void *userdata);
typedef s32  (*PFN_amw_simulation)(struct a_moonlit_walk *AMW, struct amw_workload *work, void *userdata);
typedef s32  (*PFN_amw_rendering)(struct a_moonlit_walk *AMW, struct amw_workload *work, void *userdata);
typedef s32  (*PFN_amw_gpuexec)(struct a_moonlit_walk *AMW, struct amw_workload *work, void *userdata);
typedef void (*PFN_amw_fini)(struct a_moonlit_walk *AMW, void *userdata);
...
struct amw_hints {
    ...
    u32         ipomoea_root_budget_estimate;
    u32         ipomoea_max_memory_budget;

    s32         pelagia_preferred_device_idx;
    s32         pelagia_target_workload_buffering;
...
enum amw_flags {
    ...
    amw_flag_finalize_gameloop              = (1u << 30), /**< Don't continue with more work - finish all current frames, cleanup and exit. */
...
struct a_moonlit_walk {
    ...
    struct pelagia_render_targets   render_targets; 
    struct pelagia_uniform_buffers  uniform_buffers;
