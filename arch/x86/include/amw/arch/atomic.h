...
/** 
 * @file amw/arch/atomic.h
 * @brief Atomic operation primitives on x86_64 and x86.
 */

#ifndef _AMW_ARCH_X86_ATOMIC_H
#define _AMW_ARCH_X86_ATOMIC_H
 ...
 *  memory_model_seq_cst. */
#if defined(AMW_ARCH_AMD64)
    #define at_read_explicit(src, dst, model)       \
        ({                                          \
            typeof(dst) dst__ = (dst);              \
...
#define at_read(src, dst) \
    at_read_explicit(src, dst, memory_model_seq_cst)
...
#define atomic_op__(rmw, op, arg, orig, model)                              \
    ({                                                                      \
        ...
        typeof(*(rmw)) val__;                                               \
                                                                            \
        at_read_explicit(rmw__, &val__, memory_model_relaxed);              \
        do {                                                                \
...
#define atomic_add__(rmw, arg, orig, model)             \
    ({                                                  \
        typeof(rmw) rmw__ = (rmw);                      \
...
#if defined(AMW_ARCH_AMD64)
    #define at_add_explicit(rmw, arg, orig, model)  \
        atomic_add_64__(rmw, arg, orig, model)

#else /* 32-bit */
    #define at_add_explicit(rmw, arg, orig, model)  \
        (sizeof(*(rmw)) <= 4                            \
         ? atomic_add_64__(rmw, arg, orig, model)       \
         : atomic_op__(rmw, +, arg, orig, model))
#endif

#define at_add(rmw, arg, orig) \
    at_add_explicit(rmw, arg, orig, memory_model_seq_cst)

#define at_sub_explicit(rmw, arg, orig, model) \
    at_add_explicit(rmw, -(arg), orig, model)
#define at_sub(rmw, arg, orig) \
    at_sub_explicit(rmw, arg, orig, memory_model_seq_cst)

#define at_or_explicit(rmw, arg, orig, model) \
    atomic_op__(rmw, |, arg, orig, model)
#define at_or(rmw, arg, orig) \
    at_or_explicit(rmw, arg, orig, memory_model_seq_cst)

#define at_xor_explicit(rmw, arg, orig, model) \
    atomic_op__(rmw, ^, arg, orig, model)
#define at_xor(rmw, arg, orig) \
    at_xor_explicit(rmw, arg, orig, memory_model_seq_cst)

#define at_and_explicit(rmw, arg, orig, model) \
    atomic_op__(rmw, &, arg, orig, model)
#define at_and(rmw, arg, orig) \
    at_and_explicit(rmw, arg, orig, memory_model_seq_cst)
