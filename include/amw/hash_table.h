 ...
 *  already exists in the table, and if so, the existing value is untouched. */
AMWAPI attr_nonnull(1,2)
...
/** Searches for a key inside the table, returns true if found or false otherwise. Writes a pointer
 *  to this value in out_value, or writes NULL if the key does not exist in the table. */
