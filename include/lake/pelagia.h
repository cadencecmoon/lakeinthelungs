...
struct pelagia {
    ...
    struct vulkan_textures   render_targets;        /**< Render target texture duplicated per workload. */
    struct vulkan_buffers    uniform_buffers;       /**< Buffers holding frame constants duplicated per workload. */

    struct arena_allocator   scratch_arena;         /**< A memory pool for temporary states. */
};
