...
#ifndef AMW_RIVEN_STACK_GUARD
#define AMW_RIVEN_STACK_GUARD 0
#endif
...
#if 0 // AMW_RIVEN_STACK_GUARD
#if defined(AMW_PLATFORM_LINUX)
    #include <unistd.h> // FIXME syscalls!
    #include <sys/mman.h>
...
{
#if defined(AMW_PLATFORM_LINUX)
    i32 res = mprotect(mem, bytes, PROT_NONE); // FIXME syscalls!
    assert_debug(!res);
#elif defined(AMW_PLATFORM_WINDOWS)
    DWORD ignored;
...
{
#if defined(AMW_PLATFORM_LINUX)
    i32 res = mprotect(mem, bytes, PROT_READ | PROT_WRITE); // FIXME syscalls!
    assert_debug(!res);
#elif defined(AMW_PLATFORM_WINDOWS)
    DWORD ignored;
...
static void mpmc_queue_init(struct mpmc_queue *q, 
                            ...
                            size_t cell_count)
{
    ...
    q->buffer = cells;
    q->buffer_mask = cell_count - 1; /* ((1 << log_2_tear_count) - 1) */
...
static size_t get_next_fiber(struct riven *riven)
{
    ...
    if (fiber_idx == FIBER_INVALID) {
        ...
        if (mpmc_dequeue(&riven->queue, &rip)) {
            ...
            const size_t bytes = riven->stack_bytes + AMW_RIVEN_STACK_GUARD;
...
{
    assert_paranoid(thread_count > 0);
    assert_paranoid(fiber_stack_bytes > 1);
    assert_paranoid(fiber_count >= (thread_count << 1));
    ...
    struct riven *riven = NULL;
    {
        ...
        size_t bytes_alignment      = 15 + (AMW_RIVEN_STACK_GUARD);

        stack_heap_bytes += AMW_RIVEN_STACK_GUARD * (fiber_count + 1);
...
#if AMW_RIVEN_STACK_GUARD > 0
        riven->stack = (u8 *)((((intptr_t)&raw[o]) + (AMW_RIVEN_STACK_GUARD - 1)) & ~(AMW_RIVEN_STACK_GUARD - 1));
#else
    ...
    for (size_t i = 0; i <= riven->fiber_count; i++) {
        stack_memory_guard(&riven->stack[i * (riven->stack_bytes + AMW_RIVEN_STACK_GUARD)], AMW_RIVEN_STACK_GUARD);
    }
    ...
    thread_affinity(&riven->stack[AMW_RIVEN_STACK_GUARD], riven->threads, thread_count, 0);
