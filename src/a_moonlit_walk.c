...
void amwe_hint_encore(u32 hint, PFN_riven_encore encore)
{
    ...
    case AMWE_HINT_ENCORE_HADAL:
        g_hints.encore_hadal = encore;
    case AMWE_HINT_ENCORE_XAKU:
        g_hints.encore_xaku = encore;
    case AMWE_HINT_ENCORE_SOMA:
        g_hints.encore_soma = encore;
    case AMWE_HINT_ENCORE_PELAGIAL:
        g_hints.encore_pelagial = encore;
    default: break;
    }
}
...
#undef ENCORE_ARRAY_SETUP
        ...
        for (u32 i = 0; i < 3; i++) {
            ...
            encores[i].tag = tag;
            encores[i].metadata = amwe->metadata;
...
#define ENCORE_FALLBACK(T)                                                                                  \
            if (!T.data) {                                                                                  \
                if (g_hints.null_fallback_native_encores) {                                                 \
                    ...
                    T.data = T##_encore_null(riven, amwe->riven_hints, amwe->metadata, &T##_assembly, tag); \
                }                                                                                           \
                if (!T.data) {                                                                              \
...
#undef ENCORE_FALLBACK
    ...
    g_hints.encore_pelagial(riven, amwe->riven_hints, amwe->metadata, amwe, tag);
...
#define ENCORE_ENDS(T) \
    prev_refcnt = T##_fini(&amwe->T, &ends[end_index]); \
