...
#define RIVEN_FIBER_INVALID SSIZE_MAX

/* Align memory address to 16. */
#define A16(x) (((x) + 15) & ~15)
    ...
    #define RIVEN_VALGRIND_STACK_ID u32 stack_id
...
#else
    #define RIVEN_VALGRIND_STACK_ID
...
static_assertion(sizeof(sptr) == sizeof(struct riven *));
static_assertion(sizeof(usize) >= sizeof(void *));
...
#if RIVEN_STACK_GUARD
static void stack_memory_guard(void *mem, usize bytes)
{
...
#else
#define stack_memory_guard(mem, bytes) ((void)0)
#endif /* RIVEN_STACK_GUARD */

#if RIVEN_STACK_GUARD
static void stack_memory_stop(void *mem, usize bytes)
{
#if defined(LAKE_PLATFORM_LINUX)
...
#else
#define stack_memory_stop(mem, bytes) ((void)0)
#endif /* RIVEN_STACK_GUARD */
...
/* cell of a multiple-producer multiple-consumer queue */
struct mpmc_cell;
...
static ssize get_free_fiber(struct riven *riven)
{
    for (ssize i = 0; i < riven->fiber_count; i++) {
        ...
        if (at_compare_exchange_weak_explicit(
            &riven->free[i], &expected, RIVEN_FIBER_INVALID,
...
{
    ssize fiber_idx = RIVEN_FIBER_INVALID;
    ...
    for (ssize i = 0; i < riven->fiber_count; i++) {
        ...
        if (at_compare_exchange_weak_explicit(
            &riven->waiting[i], &expected, RIVEN_FIBER_INVALID,
            memory_model_release, memory_model_relaxed))
        {
            fiber_idx = i;
            break;
    ...
    if (fiber_idx == RIVEN_FIBER_INVALID) {
        struct ripntear rip;

        if (mpmc_dequeue(&riven->queue, &rip)) {

            while (fiber_idx == RIVEN_FIBER_INVALID)
...
static at_ssize *get_lock(struct riven *riven, ssize initial_value)
{
    for (;;) {
        for (ssize i = 0; i < riven->fiber_count; i++) {
            ...
            if (at_read_explicit(lock, memory_model_relaxed) == RIVEN_FIBER_INVALID) {
                ssize expected = RIVEN_FIBER_INVALID;
    ...
    rivens_chain_t     *chain)
{
    ...
    for (ssize i = 0; i < splits; i++) {
        ...
        while (!mpmc_enqueue(&riven->queue, &rip)) { 
            assert_release(!"Failed to queue work for fibers in the multiple-producer multiple-consumer queue."); 
        };
    }
}

void riven_unchain(struct riven *riven, rivens_chain_t chain)
{
    ...
    if (counter) at_store_explicit(counter, RIVEN_FIBER_INVALID, memory_model_release);
}

void riven_acquire_chain(struct riven *riven, rivens_chain_t *chain)
{
    at_ssize **counters = chain;
    if (counters) *counters = get_lock(riven, 1);
}

ssize riven_unveil_rift(
    void          *riven_memory,
    ...
    void          *main_argument)
{
    ...
    struct riven *riven = NULL;
    {
        ...
        ssize thread_bytes         = A16(sizeof(thread_t) * thread_count);
    ...
    for (ssize i = 0; i < riven->fiber_count; i++) {
        ...
        at_store_explicit(&riven->waiting[i], RIVEN_FIBER_INVALID, memory_model_release);
        at_store_explicit(&riven->locks[i], RIVEN_FIBER_INVALID, memory_model_release);
    }
