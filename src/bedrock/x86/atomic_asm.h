 ...
 *  as the CPU guarantees such memory accessess to be atomic. */
    ...
    #define ARCH_ATOMIC(type) type volatile
#else
    #define ARCH_ATOMIC(type) type volatile ALIGN_TO(sizeof(type))
#endif
...
#define ARCH_ATOMIC_BOOL_LOCK_FREE 2
#define ARCH_ATOMIC_CHAR_LOCK_FREE 2
#define ARCH_ATOMIC_SHORT_LOCK_FREE 2
#define ARCH_ATOMIC_INT_LOCK_FREE 2
#define ARCH_ATOMIC_LONG_LOCK_FREE 2
#define ARCH_ATOMIC_LLONG_LOCK_FREE 2
#define ARCH_ATOMIC_POINTER_LOCK_FREE 2

#define _ARCH_IS_LOCKLESS_ATOMIC__(obj) \
    (sizeof(obj) <= 8 && ((sizeof(obj) != 0) && ((sizeof(obj) & (sizeof(obj) - 1)) == 0)))
...
INLINE void at_compiler_barrier(enum memory_model model) {
    if (model > memory_model_consume) {
        compiler_barrier();
    }
}

INLINE void at_thread_fence(enum memory_model model) {
...
INLINE void at_signal_fence(enum memory_model model) {
    at_compiler_barrier(model);
}

#define at_is_lock_free(obj) \
    ((void)*(obj), _ARCH_IS_LOCKLESS_ATOMIC__(*(obj)) ? 2 : 0)
 ...
 *  memory_model_seq_cst. */
    ...
    #if defined(ARCH_X86_SSE)
        /** SSE registers are 128-bit wide, and moving the lowest 64-bits of an SSE 
         *  register to properly aligned memory is atomic. See LAKE_ATOMIC(type) docs. */
...
typedef ARCH_ATOMIC(b32) at_flag;
#define ARCH_ATOMIC_FLAG_INIT { 0 }
...
#define at_clear_explicit(flag, model) \
    at_store_explicit(flag, 0, model)
#define at_clear(flag) \
    at_clear_explicit(flag, memory_model_seq_cst)
