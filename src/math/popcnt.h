...
static u8 popcnt_lookup_8bit_table[256] = {
	/*  0 */ 0, /*  1 */ 1, /*  2 */ 1, /*  3 */ 2,
	/*  4 */ 1, /*  5 */ 2, /*  6 */ 2, /*  7 */ 3,
	/*  8 */ 1, /*  9 */ 2, /*  a */ 2, /*  b */ 3,
	/*  c */ 2, /*  d */ 3, /*  e */ 3, /*  f */ 4,
...
u64 popcnt_lookup_8bit(const u8 *data, const usize n)
{
    usize result = 0;
    usize i = 0;
    while (i + 4 <= n) {
        result += popcnt_lookup_8bit_table[data[i]]; i++;
        result += popcnt_lookup_8bit_table[data[i]]; i++;
        result += popcnt_lookup_8bit_table[data[i]]; i++;
        result += popcnt_lookup_8bit_table[data[i]]; i++;
    }
...
#else
    return popcnt_lookup_8bit(data, n);
#endif
}
