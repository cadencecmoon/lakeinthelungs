...
usize bits_ffs_table_lookup(const u8 *data, usize n)
{
    for (usize o = 0; o < n; o++)
...
u64 bits_popcnt_table_lookup(const u8 *data, usize n)
{
    u64 bits = 0;
    usize o = 0;
    while (o + 4 <= n) {
...
u32 bits_log2_next_pow2(u32 n)
{
    ...
    /* compute log2, subtract 1 as ffs returns a 1-based, not 0-based value */
    return bits_ffs_table_lookup((const u8 *)&n, lake_sizeof(u32)) - 1;
}
