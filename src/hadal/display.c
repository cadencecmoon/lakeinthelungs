...
static const char *api_name_string(uint32_t id)
{
    switch (id) {
...
uint32_t hadal_should_close(struct hadal *hadal, bool should)
{
    if (!hadal)
        return 0u;
...
uint32_t hadal_visible(struct hadal *hadal, bool visible)
{
    if (!hadal)
        return 0u;
