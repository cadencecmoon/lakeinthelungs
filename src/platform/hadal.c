...
u32 hadal_api_get_current(struct hadal *hadal)
{
    if (hadal)
...
void hadal_fini(struct hadal *hadal)
{
    ...
    terminate(hadal);
    if (hadal->window.title) /* TODO replace stdlib malloc/free */
...
u32 hadal_flags(struct hadal *hadal)
{
    if (hadal)
...
u32 hadal_should_close(struct hadal *hadal, bool should)
{
    if (!hadal)
        return 0u;
    ...
    if (should) {
        return at_fetch_or_explicit(&hadal->flags, hadal_flag_should_close, memory_model_release);
    } else {
        return at_fetch_and_explicit(&hadal->flags, ~hadal_flag_should_close, memory_model_release);
    }
}

u32 hadal_visible(struct hadal *hadal, bool visible)
{
    if (!hadal)
        return 0u;
