...
/** @file moon/moon_swapchain.h
 *  @brief Used as the means to present rendered images to the window system.
 *
 *  Following interface procedures are defined:
 *      PFN_moon_swapchain_assembly
 *      PFN_moon_swapchain_zero_refcnt
 *      PFN_moon_swapchain_wait_for_next_frame
 *      PFN_moon_swapchain_acquire_next_image
 *      PFN_moon_swapchain_current_acquire_semaphore
 *      PFN_moon_swapchain_current_present_semaphore
 *      PFN_moon_swapchain_current_cpu_timeline_value
 *      PFN_moon_swapchain_current_timeline_pair
 *      PFN_moon_swapchain_gpu_timeline_semaphore
 *      PFN_moon_swapchain_set_present_mode
 *      PFN_moon_swapchain_resize
 */
#include "moon_adapter.h"
#include "moon_timeline.h"
#include "../hadal/hadal_adapter.h"
...
typedef enum : moon_present_transform {
...
/** Default way to select a surface format for the swapchain. */
LAKEAPI s32 LAKECALL moon_default_surface_format_selector(u32 format_count, moon_format *formats);

/** Details needed to create a swapchain. */
typedef struct {
    /** A window obtained from the display backend. */
    hadal_window                        native_window;
...
PFN_MOON_ASSEMBLY(swapchain, swapchain_assembly, device);
#define FN_MOON_SWAPCHAIN_ASSEMBLY(backend) \
    FN_MOON_ASSEMBLY(backend, swapchain, device)

PFN_MOON_ZERO_REFCNT(swapchain);
#define FN_MOON_SWAPCHAIN_ZERO_REFCNT(backend) \
    FN_MOON_ZERO_REFCNT(backend, swapchain)
