...
/** Log a message. If the framework is not initialized, the logger will flush immediately 
 *  to a default output stream. Otherwise, messages are saved onto a thread-local buffer 
 *  and the user needs to flush these buffers for the messages to deliver. */
LAKE_HOT_FN 
LAKEAPI void LAKECALL 
lake_log_(
    s32         level, 
...
#define lake_warn_(fn, file, line, ...) lake_log_(-2, fn, file, line, __VA_ARGS__)
...
#define lake_error_(fn, file, line, ...) lake_log_(-3, fn, file, line, __VA_ARGS__)
...
#define lake_fatal_(fn, file, line, ...) lake_log_(-4, fn, file, line, __VA_ARGS__)
...
#define lake_assert(condition, error_code, ...)                 \
    ...
    } while(LAKE_NULL_WHILE_LOOP_CONDITION)
...
/** Enable or disable the console output, if the framework is running, a file logger may still be used.
 *  @return Previous console setting. */
LAKEAPI bool LAKECALL
lake_log_enable_console(
    bool enabled);
