...
void *lake_sys_mmap(usize size_page_aligned, usize hugepage_size)
{
    void *mapped = NULL;
    ...
    lake_madvise_mode mode)
{
    ...
    char const *errtype = mode == lake_madvise_mode_commit ? "commitment" : "release";
    bool        success = 1;

    /* commit physical memory */
    if (mode == lake_madvise_mode_commit) {
        s32 res = mprotect(raw_map, size_page_aligned, PROT_READ | PROT_WRITE);
        if (res == 0)
            res = madvise(raw_map, size_page_aligned, MADV_WILLNEED);
        success = (res == 0);
    /* release physical memory */
    } else if (mode == lake_madvise_mode_release) {
