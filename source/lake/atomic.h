...
LAKE_FORCE_INLINE bool lake_spinlock_try_acquire(volatile lake_spinlock *lock)
{ return lake_atomic_flag_test_and_set_explicit(&lock->flag, lake_memory_model_acquire); }

LAKE_FORCE_INLINE void lake_spinlock_acquire(volatile lake_spinlock *lock)
{ do { /* spin */ } while(lake_spinlock_try_acquire(lock)); }

LAKE_FORCE_INLINE void lake_spinlock_release(volatile lake_spinlock *lock)
