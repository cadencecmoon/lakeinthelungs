...
struct wayland_display *g_wl_display = NULL;

HADAL_ENTRY_POINT(wayland)
{
    u32 id = hadal_backend_wayland;
    const char *name = "wayland";

    interface->id = id;
    interface->name = name;

    if (UNLIKELY(g_wl_display != NULL)) {
        assert_debug(g_wl_display->hadal);
        interface->internal = g_wl_display->hadal;
        if (verbose)
            log_debug("A Wayland display backend already exists, returning a shared instance.");
        /* we'll interpret this as internal == struct hadopelagic */
        return result_reiterate;
    }

    void *client = process_load_dll("libwayland-client.so.0");
    if (!client) {
        if (verbose)
    ...
    struct wayland_display *wl = (struct wayland_display *)riven_alloc(create_info->riven, create_info->tag, sizeof(struct wayland_display), _Alignof(struct wayland_display));
    zerop(wl);
    ...
    *interface = (struct hadopelagic_interface){
        .id = id,
        .name = name,
        .internal = g_wl_display,
        .display_init = hadal_wayland_display_init,
        .display_fini = hadal_wayland_display_fini,
        .get_framebuffer_extent = hadal_wayland_get_framebuffer_extent,
        .create_swapchain_surface = hadal_wayland_create_swapchain_surface,
    };
    ...
    u32                 version)
{
    ...
    struct wayland_display *wl = (struct wayland_display *)hadal->interface.internal;
...
s32 hadal_wayland_display_init(
    ...
    const struct hadopelagic_create_info *restrict create_info)
{
    ...
    atomic_store_explicit(&wl->fb_width, hadal->window_width, memory_order_release);
    atomic_store_explicit(&wl->fb_height, hadal->window_height, memory_order_release);

    atomic_fetch_and_explicit(&hadal->flags, hadal_flag_display_is_valid, memory_order_release);
    return result_success;
}

void hadal_wayland_display_fini(void *restrict internal)
{
    struct wayland_display *wl = (struct wayland_display *)internal;
...
void hadal_wayland_get_framebuffer_extent(
    ...
    u32                *out_height)
{
    ...
    if (out_width) 
        *out_width = atomic_load_explicit(&wl->fb_width, memory_order_acquire);
    if (out_height)
        *out_height = atomic_load_explicit(&wl->fb_height, memory_order_acquire);
}
...
s32 hadal_wayland_create_swapchain_surface(
    struct hadopelagic       *hadal, 
    ...
    struct pelagic_swapchain *swapchain)
{
#ifdef PELAGIAL_VULKAN
    struct wayland_display *wl = (struct wayland_display *)hadal->interface.internal;
    const struct vulkan_backend *api = (const struct vulkan_backend *)pelagial->interface.internal;
    struct vulkan_swapchain *chain = (struct vulkan_swapchain *)swapchain;

    assert_debug(pelagial && pelagial->interface.id == pelagic_backend_vulkan && api->instance);
    ...
    VERIFY_VK(api->vkCreateWaylandSurfaceKHR(api->instance, &surface_info, NULL, &chain->surface));
    return chain->surface ? result_success : result_error;
#else
    (void)hadal; (void)pelagial; (void)swapchain;
    log_warn("There is no Vulkan backend built-in that Wayland can interface it's surface with.");
    return result_error;
#endif
}
